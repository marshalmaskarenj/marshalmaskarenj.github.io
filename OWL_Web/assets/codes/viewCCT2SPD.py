"""
Converts CCT to SPD according to CIE other illuminants D, modified for indoor scene. Works in tandem with SpectralViewdome.
For a given CCT, this component generates the relative SPD for various wavelegths between 300-830nm, with a bin size of 5nm. 
The values are relative to 560nm, thus 560 nm refers to a value of 100 (percent).
This is derived from CIE 015: 2004.
Functionally similar to CCT2SPD but modified for indoor scene where some patches have no sky view, so luminance=0 and CCT=0.
--
OWL (Occupant Well-being through Lighting) is developed by Marshal Maskarenj, for SCALE project funded by FNRS @ LOCI, UCLouvain.
    Inputs:
        CCT: The correlated color temperature of sky.
    Output:
        SPD: Relative Spectral Power Distribution at defined wavelengths."""

self=ghenv.Component
self.Name = "SCALE_viewCCT2SPD"
self.NickName = 'viewCCT2SPD'
...
#    CCT=5000 if CCT is None else CCT
    if 100<CCT<4000:
        CCT=4000
    if CCT>25000:
        CCT=25000
    ...
    if CCT==0:
        xD=0
        yD=0
        M0=0 #a factor to neutralise the S0 array
        M1=0
        M2=0
    else:
        xD= xDcalc(CCT)
    ...
    print CCT,"      ",xD,"        ",yD,"        ",M1,"        ",M2
    SPDarr=[]
    for i in range(0,len(wav_arr),1):
        SPDval=M0*s0_arr[i]+M1*s1_arr[i]+M2*s2_arr[i]
